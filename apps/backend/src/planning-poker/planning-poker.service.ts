...
export class PlanningPokerService {
  ...
  createRoom(payload: CreateRoomPayload): { room: Room; user: User } {
    ...
    this.logger.log(`Room created: ${roomId} by user ${userId}`);
  ...
  leaveRoom(userId: string, roomId: string): Room | null {
    ...
    if (room.participants.length === 0) {
      ...
      this.logger.log(`Room ${roomId} deleted (no participants left)`);
      return null;
    }
    ...
    if (room.participants.every(p => p.role !== UserRole.MODERATOR)) {
      ...
      this.logger.log(`New moderator assigned in room ${roomId}: ${newModerator.id}`);
    }
  ...
  createStory(payload: CreateStoryPayload): Room | null {
    ...
    this.logger.log(`Story created: ${storyId} in room ${roomId}`);
    return room;
  }
  ...
  updateStory(payload: UpdateStoryPayload): Room | null {
    ...
    this.logger.log(`Story updated: ${storyId} in room ${roomId}`);
    return room;
  }
  ...
  deleteStory(roomId: string, storyId: string): Room | null {
    ...
    this.logger.log(`Story deleted: ${storyId} from room ${roomId}`);
  ...
  setActiveStory(payload: SetActiveStoryPayload): Room | null {
    ...
    this.logger.log(`Active story set: ${storyId} in room ${roomId}`);
    return room;
  }
  ...
  submitVote(payload: SubmitVotePayload): Room | null {
    ...
    this.logger.log(`Vote submitted by user ${userId} in room ${roomId}`);
    return room;
  }
  ...
  revealVotes(roomId: string): Room | null {
    ...
    this.logger.log(`Votes revealed in room ${roomId}`);
  ...
  resetVotes(roomId: string): Room | null {
    ...
    this.logger.log(`Votes reset in room ${roomId}`);
    return room;
  }
